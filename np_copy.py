...
if __name__ == "__main__":
    ...
    # End Time Forecast
    # data_aggregated_end = data.groupby('FULL_DATETIME_STRT').agg({
    #     'ENDTIME_OLD': 'first'
    # }).reset_index()
    # data_aggregated_start.to_csv('data_merged.csv')
    
    

    # Combine DataFrames based on index
    # data_merged = pd.concat([data_aggregated_start, data_aggregated_end], axis=1)
    
    # # Rename columns if necessary
    # data_merged.columns = ['FULL_DATETIME_STRT', 'STRTTIME_OLD','FULL_DATETIME_END', 'ENDTIME_OLD']

    # print(len(data_merged))
    # print("NaN values in ds column:")
    # print(data_merged['FULL_DATETIME_STRT'].isna().sum())
    # df_cleaned = data_merged.dropna(subset=['FULL_DATETIME_STRT'])

    # print("Number of rows after dropping NaNs:", len(df_cleaned))
    #     # Check for NaN values
    data_merged = data_aggregated_start
    
    # Step 4: Prepare Data
    df_strttime = prepare_data(data_merged, 'FULL_DATETIME_STRT', 'STRTTIME_OLD')
    df_endtime = prepare_data(data_merged, 'FULL_DATETIME_STRT', 'ENDTIME_OLD')

    print(df_strttime)
    print(df_endtime)
    
    # Train-test split for start time
    train_df_strttime, test_df_strttime = train_test_split(df_strttime)

    # Train NeuralProphet model for start time
    model_strttime, _ = train_neuralprophet(train_df_strttime)

    # Forecast on the test set for start time
    forecast_strttime = model_strttime.predict(test_df_strttime)

    # Evaluate the model for start time
    mae_strttime, mse_strttime, rmse_strttime = evaluate_model(test_df_strttime, forecast_strttime)
    print(f'Start Time Forecast - MAE: {mae_strttime}, MSE: {mse_strttime}, RMSE: {rmse_strttime}')

    # Forecast next day for start time
